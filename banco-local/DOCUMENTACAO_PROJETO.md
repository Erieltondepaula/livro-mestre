...
### v1.7 (25 Janeiro 2026)
- **Vínculo Dinâmico de Palavras**: Palavras podem ser salvas sem livro e vinculadas posteriormente
  - Salvar palavras como "Outro" (sem livro vinculado)
  - Botão "Editar vínculo" no diálogo de detalhes da palavra
  - Vincular/desvincular livros a qualquer momento
  - Campo opcional de página ao vincular
  - Atualização automática da lista após edição
- **Pasta `banco-local/`**: Criada estrutura para scripts de migração local
  - Contém migrations incrementais versionadas
  - README com instruções de uso
  - Cópia da documentação do projeto
...
## 13. Scripts para Banco Local

A pasta `banco-local/` contém todos os scripts necessários para sincronizar seu banco Supabase externo:

```
banco-local/
├── README.md                         # Instruções de uso
├── migrations/                       # Migrações incrementais
│   └── v1.7_vinculo_palavras.sql    # Última migração
└── (schema-completo.sql)            # Para instalação limpa
```

### Como Atualizar Seu Banco Local
1. Navegue até `banco-local/migrations/`
2. Execute os scripts SQL que ainda não foram aplicados
3. Scripts são idempotentes (podem ser executados múltiplas vezes sem problemas)
