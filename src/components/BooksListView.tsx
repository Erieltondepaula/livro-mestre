...
import { Edit2, Trash2, BookOpen, ChevronDown, ChevronRight, Filter, Star, Clock, Calendar } from 'lucide-react';
...
export const BooksListView = forwardRef<HTMLDivElement, BooksListViewProps>(function BooksListView({ books, statuses, readings, evaluations, onDeleteBook, onUpdateBook, initialFilter = 'all' }, ref) {
  ...
  // Calcula a média de páginas por minuto
  ...
  // Calcula a data prevista de conclusão baseado na média diária de leitura
  const getEstimatedCompletionDate = (book: Book, status: BookStatus | undefined): Date | null => {
    ...
    // Busca leituras dos últimos 30 dias para calcular média diária
    ...
    if (recentReadings.length === 0) {
      // Sem histórico recente, estima 10 páginas por dia como padrão
    ...
    // Calcula média de páginas por dia de leitura
  ...
  const getStatusInfo = (book: Book, status: BookStatus | undefined): { text: string; colorClass: string } => {
    if (!status || status.status === 'Não iniciado') {
      return { text: 'Não iniciado', colorClass: 'text-muted-foreground' };
    }
    
    if (status.status === 'Concluido') {
      return { text: 'Concluído', colorClass: 'text-green-600' };
    }
    
    const percentRead = (status.quantidadeLida / book.totalPaginas) * 100;
    
    if (percentRead > 50) {
      return { text: '', colorClass: 'text-orange-500' };
    }
    
    return { text: '', colorClass: 'text-blue-600' };
  };
    ...
    return (
      <TooltipProvider key={book.id}>
        ...
        >
          ...
          {/* Informações do Livro - Compactas */}
          <div className="p-2 space-y-1 flex-1 flex flex-col">
            {/* Badge de Avaliação ACIMA do título */}
            {evaluation && (
              <div className="flex items-center gap-1">
                ...
                <div className="flex">
                  {[1, 2, 3, 4, 5].map((star) => (
                    <Star
                      key={star}
                      className={`w-2.5 h-2.5 ${star <= Math.round(evaluation.notaFinal / 2) ? 'fill-amber-400 text-amber-400' : 'text-gray-300'}`}
                    />
                  ))}
                </div>
              </div>
            ...
            <Tooltip>
              ...
              <TooltipContent side="top" className="max-w-xs">
                <p className="font-semibold">{book.livro}</p>
                {book.autor && <p className="text-muted-foreground text-sm">{book.autor}</p>}
                {book.ano && <p className="text-muted-foreground text-xs">Ano: {book.ano}</p>}
                <p className="text-muted-foreground text-xs">Tipo: {book.tipo}</p>
                {book.categoria && <p className="text-muted-foreground text-xs">Categoria: {book.categoria}</p>}
                <p className="text-muted-foreground text-xs">Páginas: {book.totalPaginas}</p>
              </TooltipContent>
            </Tooltip>
            
            {/* Autor */}
            {book.autor && (
              <p className="text-[10px] text-muted-foreground truncate">{book.autor}</p>
            )}

            {/* Status/Tempo */}
            <div className="mt-auto pt-1">
              {status?.status === 'Concluido' ? (
                <span className={`text-[10px] font-medium ${statusInfo.colorClass}`}>
                  ✓ {statusInfo.text}
                </span>
              ) : totalMinutes ? (
                ...
                <span className={`text-[10px] ${statusInfo.colorClass}`}>{statusInfo.text}</span>
              ) : null}
            </div>
          </div>
        </div>
