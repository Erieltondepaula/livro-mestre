...
async function fetchVerseText(ref: string): Promise<string> {
  ...
  const verseSegment = match[3].replace(',', '-');
  const verseRef = `${bookSlug}+${chapter}:${verseSegment}`;

  // 1) Try bible-api in Portuguese (Almeida)
  try {
  ...
  // 2) Fallback to backend proxy that scrapes ACF directly (Portuguese only)
  const acfBook = BIBLE_ONLINE_SLUGS[bookName];
  const rangeMatch = verseSegment.match(/^(\d+)(?:-(\d+))?$/);
  if (!acfBook || !rangeMatch) return '';

  const verseStart = Number(rangeMatch[1]);
  const verseEnd = Number(rangeMatch[2] ?? rangeMatch[1]);

  try {
    const { data, error } = await supabase.functions.invoke('acf-verse', {
      body: {
        book: acfBook,
        chapter,
        verseStart,
